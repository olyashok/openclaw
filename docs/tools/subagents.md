...
- **Parent attachments:** files attached to the requester's current turn (for example PDFs a user asked to compare) are copied into the native or visible child's workspace under `.openclaw/attachments/` and listed, as workspace-relative paths, at the end of the child's task. Files over 50 MB, or past 200 MB in total, are listed as not forwarded. ACP spawns do not receive them.
