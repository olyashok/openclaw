...
<Tabs>
  ...
  <Tab title="Mentions and channel users">
    ...
    `requestUsers` separates collaboration from general request authority. When omitted, admitted channel users keep normal behavior. When configured, only listed stable Slack user IDs (or `"*"`) have general request authority, may run slash commands, or may use interactive action surfaces. Other users admitted by `users` normally contribute `room_event` context, even when they mention the bot or send control/abort text. Slack adds a persistent :blue_book: reaction when that context reaches the agent; it means “captured as context,” not “authorized as an instruction.”

    There is one thread-scoped delegation exception: after a listed `requestUsers` member starts or participates in a thread, an admitted context-only participant's later reply is dispatched with a strict continuation-only policy. The agent may act only when the reply continues, clarifies, or corrects the established thread scope. An expanded or unclear action must receive a visible refusal explaining that a listed requester must approve it. This never grants slash-command, reset, abort, authorization-change, or general channel request authority. OpenClaw verifies the earlier authorized participant from Slack's thread authorship metadata and fails closed when it cannot read that metadata. An empty list makes every admitted user context-only because no thread can acquire an authority anchor. This setting remains restrictive: it never admits a sender excluded by `users` or the surrounding channel policy.
...
- When separate Slack bot accounts intentionally share a channel, `channels.slack.threadOwnership.preferredAccounts` makes an unmentioned thread continuation single-owner. OpenClaw selects the first already-participating account in that order, persists the choice across gateway restarts, and suppresses the other accounts before they create a turn. An explicit mention of another bot transfers ownership to that bot. When one message mentions several of these bots, only the highest-ranked mentioned account runs a turn. Top-level channel messages follow the same rules and seed the owner of the thread they start: a mention picks its bot, and an unmentioned message that several accounts would answer (`requireMention: false`) goes to the highest-ranked of them. Claims for one thread are serialized, so concurrent handlers always agree on the owner. This is opt-in; without it, participation remains account-local.
...
```json5
{
  channels: {
    slack: {
      threadOwnership: {
        preferredAccounts: ["fi-admin", "fi-user"],
      },
    },
  },
}
