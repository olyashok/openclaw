...
// copied into the child workspace and listed in the child's task message, so
// the child transcript names them and the child can open them itself.
...
describe("spawnSubagentDirect parent-turn attachments", () => {
  ...
  it("copies the parent turn's PDFs into the child workspace and lists them in the child task", async () => {
    ...
    expect(message).toContain("2 file(s) copied into your workspace");
    const refs = [
      ...message.matchAll(
        /(\.openclaw\/attachments\/[0-9a-f-]{36}\/(Terra Legacy plumbing [AB]\.pdf)) \(application\/pdf\)/g,
      ),
    ];
    ...
    expect(fs.readFileSync(path.join(workspaceDir, refs[0]![1]), "utf8")).toBe("%PDF-1.7 A");
    expect(fs.readFileSync(path.join(workspaceDir, refs[1]![1]), "utf8")).toBe("%PDF-1.7 B");
