...
- **Parent attachments:** files attached to the requester's current turn (for example PDFs a user asked to compare) are copied for the child and listed at the end of its task. Native children read them from the same Gateway-owned attachment store as `attachments` (the `/openclaw/attachments` mount inside a sandbox, which needs a session- or agent-scoped sandbox with read-only mounts) and they are removed with the run; visible sessions get them in the child workspace under `.openclaw/attachments/`. Files over 50 MB, or past 200 MB in total, are listed as not forwarded. ACP spawns and cloud placements do not receive them.
