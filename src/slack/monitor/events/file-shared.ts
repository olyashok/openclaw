...
writeSlackDiag("diag init: file-shared.ts loaded");
...
export function registerSlackFileSharedEvents(params: {
  ...
  (
    ...
    try {
      ...
      writeSlackDiagKv("diag slack file_shared ingress", {
        ch: fileEvent.channel_id ?? "?",
      ...
      if (ctx.shouldDropMismatchedSlackEvent(body)) {
        writeSlackDiagKv("diag slack file_shared drop(mismatch)", {
          ch: fileEvent.channel_id ?? "?",
          file: fileEvent.file_id ?? "?",
        });
        return;
      ...
      if (!channelId || !fileId) {
        writeSlackDiag("diag slack file_shared drop(missing channel or file id)");
        return;
      }

      // Skip if the bot itself shared the file
      if (fileEvent.user_id === ctx.botUserId) {
        writeSlackDiagKv("diag slack file_shared drop(bot user)", { ch: channelId, file: fileId });
        return;
      }
      ...
      if (!result.ok || !result.messages?.length) {
        writeSlackDiagKv("diag slack file_shared history empty", { ch: channelId, file: fileId });
        return;
      }
      ...
      if (!matchingMessage) {
        writeSlackDiagKv("diag slack file_shared no matching message in history", {
          ch: channelId,
          file: fileId,
        });
        return;
      ...
      if (msgTs && handledMessageTimestamps.has(msgTs)) {
        writeSlackDiagKv("diag slack file_shared skipped duplicate", {
          ch: channelId,
          file: fileId,
          ts: msgTs,
        });
      ...
      if (matchingMessage.bot_id || matchingMessage.user === ctx.botUserId) {
        writeSlackDiagKv("diag slack file_shared drop(bot message)", {
          ch: channelId,
          file: fileId,
      ...
      writeSlackDiagKv("diag slack file_shared processing", {
        ch: channelId,
        ts: msgTs ?? "?",
        file: fileId,
      ...
      writeSlackDiagKv("diag slack file_shared handled", {
        ch: channelId,
        ts: msgTs ?? "?",
        file: fileId,
      });
    } catch (err) {
      writeSlackDiag(`diag slack file_shared handler exception: ${String(err)}`);
